    ...
    public abstract class FPEInteractableBaseScript : MonoBehaviour
    {
        ...
        private void setHighlightMaterial()
        {
            ...
            if (!highlightMaterialSet && highlightOnMouseOver)
            {
                ...
                // I don't know if this is required any more as of Unity 5. Keeping for posterity because it doesn't break anything ^_^
                MeshFilter[] childMeshFilters = gameObject.GetComponentsInChildren<MeshFilter>();
                MeshFilter tempMF;

                for (int mf = 0; mf < childMeshFilters.Length; mf++)
                {

                    tempMF = childMeshFilters[mf];
                    if (tempMF && tempMF.mesh && !gameObject.isStatic)
                    {
                        tempMF.mesh.RecalculateBounds();
                    }
